...
for k=1:9
    ...
    for i=1:1
        ...
        initial_condition(:)=0;
        output_sequence_list{k} = input2outputSeq(inputnode_list,outputnode_list,input_list,logic_list,initial_condition,input_sequence);
    end
    
    for i=1:n_node
        d=D(inputnode_list(k),i);
        if(~isinf(d))
            mi_list(i,k)=mutInfo(output_sequence_list{k}(end-100+1:end,i),output_sequence_list{k}(end-100+1-d:end-d,inputnode_list(k)));
        end
...
% for i=1:n_node
%     i
%     data=[output_sequence_list{k}(1:99,input_list{i}) output_sequence_list{k}(2:100,i)];
