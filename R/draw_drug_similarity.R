...
yeast.table <- read.table("/data/platform_scripts/R/drug_similarity.txt", header=T)
...
sel <- "sel"
drugs <- strsplit(args[1], split=",")
drugs <- drugs[[1]]

for(i in 1:length(drugs)){
	temp <- strsplit(drugs[i], split="_")[[1]]
	temp <- temp[2:length(temp)]
	temp2 <- temp[1]
	if(length(temp) > 1){
		for(j in 2:length(temp)){
			temp2 <- paste(temp2,temp[j],sep="_")
		}
	}
	drugs[i] <- temp2
}
...
bin <- content(a,'raw')
writeBin(bin,paste('/data/ui_input/',args[2],'drugs.png',sep=""))
