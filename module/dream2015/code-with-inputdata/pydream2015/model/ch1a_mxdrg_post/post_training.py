...
def test_run_training(with_small, overwrite): 

    run_training('output0718.json', 'score0718.json', 'traintest_div.json', 
...
def run_training(inpfile, outfile, traintest_div, with_small=False, 
    ...
    file1 = os.path.abspath( join('../mxdrg/datafiles', \
        pathsplit(workdir)[1],'STEP1.json') )

    file2 = os.path.abspath( join('../mxdrg/datafiles', \
        pathsplit(workdir)[1],'STEP2.json') )

    step1data = json.load(open(file1))
    step2data = json.load(open(file2))
    ...
    shutil.copy(join('../..', 'druggroups.csv'), '.')
    ...
    # KGROUP = 'DRUG_KGROUP:10'
    update_therapy_data_with_kdrug('THERAPY_TRAINSET.CSV', KGROUP)
    update_therapy_data_with_kdrug('THERAPY_TESTSET.CSV', KGROUP)
...
def calc_randomeffect2(therapyAll):
    ...
    rndeff_tissue = {'mean':{}, 'std':{}, 'points':{}} 
    therapy_groups = therapyAll.groupby('Tissue..General.').groups
    for group_key in therapy_groups:
        ids = therapy_groups[group_key]
        syn_mean = therapyAll.loc[ids,'SYNERGY_SCORE'].mean()
        syn_std = therapyAll.loc[ids,'SYNERGY_SCORE'].std() 
        syn_count = therapyAll.loc[ids,'SYNERGY_SCORE'].count()
        rndeff_tissue['mean'][group_key] = syn_mean
        rndeff_tissue['std'][group_key] = syn_std
        rndeff_tissue['points'][group_key] = syn_count
        pass

    rndeff_kgroup = {'mean':{}, 'std':{}, 'points':{}} 
    therapy_groups = therapyAll.groupby(KGROUP).groups
    for group_key in therapy_groups:
        ids = therapy_groups[group_key]
        syn_mean = therapyAll.loc[ids,'SYNERGY_SCORE'].mean()
        syn_std = therapyAll.loc[ids,'SYNERGY_SCORE'].std() 
        syn_count = therapyAll.loc[ids,'SYNERGY_SCORE'].count()
        rndeff_kgroup['mean'][group_key] = syn_mean
        rndeff_kgroup['std'][group_key] = syn_std
        rndeff_kgroup['points'][group_key] = syn_count
        pass

    rndeff_kdrug_tissue_id = {'mean':{}, 'std':{}, 'points':{}} 
    therapy_groups = therapyAll.groupby(KGROUP).groups
    for group_key in therapy_groups:
        ids = therapy_groups[group_key]
        syn_mean = therapyAll.loc[ids,'SYNERGY_SCORE'].mean()
        syn_std = therapyAll.loc[ids,'SYNERGY_SCORE'].std() 
        syn_count = therapyAll.loc[ids,'SYNERGY_SCORE'].count()
        rndeff_kdrug_tissue_id['mean'][group_key] = syn_mean
        rndeff_kdrug_tissue_id['std'][group_key] = syn_std
        rndeff_kdrug_tissue_id['points'][group_key] = syn_count
        pass
